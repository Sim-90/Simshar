...
def k_means_gpu(weight_vector, n_clusters, verbosity=0,
                save_path=None, seed=int(time.time()), gpu_id=7):

    N, D = weight_vector.shape

    # ---------- Case 1: Single cluster ----------
    if n_clusters == 1:
        mean_sample = np.mean(weight_vector, axis=0, keepdims=True)
        weight_vector_compress = np.repeat(mean_sample, N, axis=0)
    ...
    # ---------- Case 2: One cluster per sample ----------
    elif N == n_clusters:
        weight_vector_compress = weight_vector.copy()
    ...
    # ---------- Case 3: 1D vectors → CPU fallback ----------
    elif D == 1:
        weight_vector_compress, centers, labels = k_means_cpu(
            weight_vector, n_clusters, seed=seed
        )
        return weight_vector_compress, centers, labels

    # ---------- Case 4: Normal GPU K-Means ----------
    else:
        init_centers, _ = kmeans_plusplus(
            X=weight_vector,
            n_clusters=n_clusters,
        ...
        weight_vector_compress = centers[labels]
